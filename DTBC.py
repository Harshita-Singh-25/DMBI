...
# ========================================
# 0. LOAD DATA IN GOOGLE COLAB
# ========================================
try:
    from google.colab import files
    print("Please upload the Breast Cancer Wisconsin (Diagnostic) dataset file (e.g., 'data.csv')")
    uploaded = files.upload()
    
    # Get the filename of the uploaded file
    DATASET_FILE = list(uploaded.keys())[0]
    print(f"\nUploaded file: {DATASET_FILE}")
    
    # Read the uploaded file into a DataFrame
    df = pd.read_csv(io.BytesIO(uploaded[DATASET_FILE]))

except ImportError:
    print("Not running in Google Colab. Please load 'df' manually.")
    # Fallback for non-Colab environments (optional):
    # df = pd.read_csv('your_local_path_to_data.csv')
    # For this script to run, 'df' must be defined.
    # As a placeholder, we'll stop if not in Colab and file isn't loaded.
    if 'df' not in locals():
        raise EnvironmentError("File loading failed. This script is designed for Google Colab.")
